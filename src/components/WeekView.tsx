  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <p className="text-xs text-stone-500 font-medium">Tap a day to add or inspect tasks</p>
      </div>

      {/* Progressive columns: 1 → 2 → 3 → 7 */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-7 gap-3">
        {weekDays.map((day) => {
          ...
          return (
            <div
              key={day.dateISO}
              className={`flex flex-col min-h-[220px] md:min-h-[320px] rounded-2xl p-3 border transition ${
                isToday
            ...
            >
              ...
              <div className="flex items-center justify-between pb-2.5 mb-2 border-b border-stone-100">
                <div>
                  <span
                    className={`text-xs font-bold uppercase tracking-wider ${isToday ? 'text-amber-700' : 'text-stone-500'}`}
                  >
                    {day.dayName}
                  </span>
                  <div
                  ...
                  {dayLoad > 0 && (
                    <div
                      className={`text-[10px] font-semibold mt-1 ${overBudget ? 'text-amber-800' : 'text-stone-500'}`}
                    >
              ...
              <div className="flex-1 space-y-2 overflow-y-auto max-h-[280px] pr-1">
                {dayTodos.length === 0 ? (
                  <div className="h-20 flex items-center justify-center text-[11px] text-stone-500 italic">
                    No tasks
                  </div>
                ) : (
                  dayTodos.map((todo) => (
                    <div
                      ...
                      className={`p-2.5 rounded-xl border text-xs transition ${
                        selectedTodoId === todo.id
                          ...
                          : todo.completed
                            ? 'bg-stone-50 border-stone-100 text-stone-500 line-through'
                            : 'bg-stone-50/80 hover:bg-stone-100 border-stone-200/60 text-stone-800'
                      }`}
                    >
                      <div className="flex items-start gap-1">
                        <label className="tap-target flex-shrink-0 -ml-2 -mt-1 cursor-pointer">
                          <input
                            type="checkbox"
                        ...
                        <button
                          ...
                          className="flex-1 text-left line-clamp-2 leading-snug min-h-11 py-2 focus:outline-none"
                        >
                          {todo.title}
                        </button>
                      </div>
              ...
              <div className="pt-2 mt-2 border-t border-stone-100">
                {addingDayISO === day.dateISO ? (
                  <form onSubmit={(e) => handleDaySubmit(e, day.dateISO)} className="space-y-1.5">
                    <input
                      id={`week-add-${day.dateISO}`}
                      type="text"
                      placeholder="Task title..."
                      value={newDayTitle}
                      onChange={(e) => setNewDayTitle(e.target.value)}
                      autoFocus
                      aria-label={`New task for ${day.dayName} ${day.dateNum}`}
                    ...
                    <div className="flex items-center justify-end gap-1">
                      <button
                        ...
                        className="text-[10px] text-stone-500 hover:text-stone-700 min-h-9 px-2"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        className="text-[10px] bg-amber-700 text-white font-semibold min-h-11 px-2.5 rounded-md"
                      >
                        Add
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full min-h-11 py-2 text-[11px] text-stone-500 hover:text-amber-700 hover:bg-amber-50/50 rounded-lg transition flex items-center justify-center gap-1 font-medium"
                  >
